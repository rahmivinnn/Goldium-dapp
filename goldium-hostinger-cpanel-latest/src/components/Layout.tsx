...
export const Layout: FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-black relative">
      ...
      {/* Header */}
      <header className="bg-black/80 backdrop-blur-md border-b border-yellow-500/30 sticky top-0 z-50 animate-morphing">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            {/* Logo */}
            <Link href="/" className="flex items-center space-x-2 group hover:scale-105 transition-all duration-300">
              <div className="relative">
                <img src="/logo.jpg" alt="Goldium Logo" className="w-10 h-10 rounded-lg shadow-lg shadow-yellow-500/30 group-hover:shadow-yellow-500/60 transition-all duration-300" />
                <div className="absolute inset-0 bg-gradient-to-r from-yellow-400/20 to-orange-400/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-pulse-glow"></div>
              </div>
              <div className="hidden sm:block">
                <h1 className="text-xl font-bold bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent group-hover:animate-shimmer">
                  Goldium.io
                </h1>
                <p className="text-xs text-gray-300 group-hover:text-yellow-300 transition-colors duration-300">The Golden Future</p>
              </div>
            </Link>
              ...
              {navigation.map((item) => (
                <Link
                  ...
                  className={`group flex items-center space-x-1 px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 relative overflow-hidden ${
                    isActive(item.href)
                      ? 'bg-yellow-500/20 text-yellow-400 border-b-2 border-yellow-500 shadow-lg shadow-yellow-500/30 animate-cyber-pulse'
                      : 'text-gray-300 hover:text-yellow-400 hover:bg-yellow-500/10 hover:shadow-lg hover:shadow-yellow-500/20'
                  }`}
                >
                  ...
                  <span className="relative z-10 group-hover:scale-110 transition-transform duration-300 group-hover:animate-twinkle">{item.icon}</span>
            ...
            <div className="flex items-center space-x-4">
              <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-lg blur opacity-30 group-hover:opacity-60 transition duration-300 animate-pulse-glow"></div>
                <WalletMultiButtonDynamic className="relative bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white font-medium py-2 px-4 rounded-lg transition-all duration-300 shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/50 hover:scale-105 animate-morphing" />
              </div>
              
              {/* Mobile menu button */}
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="md:hidden p-2 rounded-md text-gray-300 hover:text-yellow-400 hover:bg-yellow-500/10 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-yellow-500/20 animate-cyber-pulse"
              >
                <svg className="w-6 h-6 hover:animate-twinkle" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Mobile Navigation */}
        {isMobileMenuOpen && (
          <div className="md:hidden bg-black/90 border-t border-yellow-500/30 backdrop-blur-md animate-morphing">
            <div className="px-2 pt-2 pb-3 space-y-1">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`group flex items-center space-x-2 px-3 py-2 rounded-md text-base font-medium transition-all duration-300 relative overflow-hidden ${
                    isActive(item.href)
                      ? 'bg-yellow-500/20 text-yellow-400 shadow-lg shadow-yellow-500/30 animate-cyber-pulse'
                      : 'text-gray-300 hover:text-yellow-400 hover:bg-yellow-500/10 hover:shadow-lg hover:shadow-yellow-500/20'
                  }`}
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-500/0 to-orange-500/0 group-hover:from-yellow-500/10 group-hover:to-orange-500/10 transition-all duration-300 animate-hologram"></div>
                  <span className="relative z-10 group-hover:scale-110 transition-transform duration-300 group-hover:animate-twinkle">{item.icon}</span>
      ...
      {/* Footer */}
      <footer className="bg-gray-900/90 text-white backdrop-blur-md border-t border-yellow-500/20 animate-morphing">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {/* Brand */}
            <div className="col-span-1 md:col-span-2">
              <div className="flex items-center space-x-2 mb-4 group">
                <div className="relative w-8 h-8 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-lg flex items-center justify-center shadow-lg shadow-yellow-500/30 group-hover:shadow-yellow-500/60 transition-all duration-300 animate-pulse-glow">
                  <img src="/logo.jpg" alt="Goldium Logo" className="w-8 h-8 object-contain" />
                  <div className="absolute inset-0 bg-gradient-to-r from-yellow-400/20 to-orange-400/20 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-shimmer"></div>
                </div>
                <h3 className="text-xl font-bold bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent group-hover:animate-cyber-pulse">Goldium.io</h3>
              </div>
              ...
              <div className="flex space-x-4">
                <a href="https://twitter.com/goldium_io" target="_blank" rel="noopener noreferrer" className="group text-gray-400 hover:text-yellow-400 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-yellow-500/30">
                  <svg className="w-6 h-6 group-hover:animate-twinkle" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                  </svg>
                </a>
                <a href="https://discord.gg/goldium" target="_blank" rel="noopener noreferrer" className="group text-gray-400 hover:text-yellow-400 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-yellow-500/30">
                  <svg className="w-6 h-6 group-hover:animate-twinkle" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a href="https://t.me/goldium_io" target="_blank" rel="noopener noreferrer" className="group text-gray-400 hover:text-yellow-400 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-yellow-500/30">
                  <svg className="w-6 h-6 group-hover:animate-twinkle" fill="currentColor" viewBox="0 0 24 24">
